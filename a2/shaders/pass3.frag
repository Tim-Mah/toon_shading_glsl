...

{
    ...
    // YOUR CODE HERE
    const mediump float threshold = 0.08;
    if (depth > 1.0 && laplacian < threshold)
    {
        discard;
    }
    ...
    // Looks more cartoony with numQuanta = 2.0
    const mediump float numQuanta = 3.0;

    // YOUR CODE HERE

    // N dot L
    mediump float diffuse = dot(normalize(normal), lightDir);

    diffuse = int(diffuse * numQuanta)/ (numQuanta-1);
    if (diffuse < 0.2) diffuse = 0.2;

    // Apply diffuse to the colour
    mediump vec3 celColour = diffuse * colour;
    ...
    // YOUR CODE HERE
    const mediump float kernelRadius = 2.0;
    mediump float minDistance = kernelRadius;

    //Check each offset, then multiply by texCoordInc to get actual position
    for (int i = -int(kernelRadius); i <= int(kernelRadius); i++) {
        for (int j = -int(kernelRadius); j <= int(kernelRadius); j++) {

            // offset in terms of spaces between texels
            mediump vec2 offset = vec2(i, j) * texCoordInc;

            // sample the neighbouring laplacian
            mediump float neighbourLaplacian = texture(laplacianSampler, texCoords + offset).r;

            // distance to closest one
            if (neighbourLaplacian > threshold) {
                // current frag is 0,0. Therefore neighbour frag has distance the mag(location)
    ...
    mediump float blendFactor = minDistance / kernelRadius;
    ...
    // YOUR CODE HERE
    mediump vec3 fragColor = celColour * blendFactor + vec3(0.0, 0.0, 0.0) * (1-blendFactor);
